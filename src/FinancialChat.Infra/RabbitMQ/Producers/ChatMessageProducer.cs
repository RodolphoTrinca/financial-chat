...
{
    public class ChatMessageProducer : IChatMessageProducer , IDisposable
    {
        ...
        public ChatMessageProducer(ILogger<StockRequestProducer> logger, IOptions<RabbitMQConfiguration> optionsConfiguration, IOptions<RabbitMQQueueNames> optionsQueueNames, IRabbitMQConnectionFactory connectionFactory)
        {
            _configuration = optionsConfiguration.Value;
            _queueName = optionsQueueNames.Value.ChatMessages;
            _logger = logger;
            _connection = connectionFactory.CreateChannel();
        }

        public bool SaveUserMessage(MessagesData messageModel)
        {
            try
            {
